...
class UI:
    ...
    def create_random_graph(self):
        ...
        if valid_num_edges(num_edges, num_vertices):
            ...
            print("Number of edges > (num_vertices * (num_vertices - 1))//2")
    ...
    def display_degree_of_vertex(self):
        new_vertex = input("Enter the vertex for displaying its degree: ")
        if valid(new_vertex):
            new_vertex = int(new_vertex)
            print("The degree of ", new_vertex," is ", self.graph.get_vertex_degree(new_vertex))

    def display_neighbors_of_vertex(self):
        new_vertex = input("Enter the vertex for displaying its neighbors: ")
        if valid(new_vertex):
            new_vertex = int(new_vertex)
            string = ""
            print("The neighbors of ",new_vertex," are: ")
            for vertex in self.graph.parse_neighbours_for_vertex(new_vertex):
                string += str(vertex) + ' '
            print(string)
    ...
    def display_connected_components(self):
        connected_components = dfs_connected_components(self.graph)
        count = 0
        for component in connected_components:
            count+=1
            print("Component", count, ":")
            if component.get_number_of_edges == 0:
                print("Isolated node ",component.vertices[0],'\n')
            else:
                print(str(component))
    ...
    def print_menu():
        ...
        13. Display the degree of a given vertex.
        14. Display the neighbors of a given vertex.
        15. Copy the graph.
        16. Create a list with connected components.
        17. Exit.\n""")
    ...
    def start(self):
        commands = {"1": self.create_random_graph, "2": self.read_graph_from_a_file,"3":self.add_vertex,
                    ...
                    "13":self.display_degree_of_vertex,"14":self.display_neighbors_of_vertex,"15":self.copy_current_graph,"16":self.display_connected_components}
        while True:
            self.print_menu()

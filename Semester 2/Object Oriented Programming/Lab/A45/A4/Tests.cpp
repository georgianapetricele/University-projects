...
{
	DynamicVector *vector1 = new DynamicVector(1);
	...
	vector1->remove(tutorial1);
	assert(vector1->get_size() == 1);
	vector1->add(tutorial1);
	vector1->update(tutorial1, 0);
	assert(vector1->get_element_on_position(0).get_title() == "Learn C++");
	*/
...
void test_repo()
{
	DynamicVector vector1 = DynamicVector(1);
	...
	assert(repo->update_tutorial(tutorial3, tutorial_auxiliary) == -1);
...
void test_ui()
{
	Ui* ui = new Ui();
	//ui->add_tutorial_ui();
	//ui->delete_tutorial_ui();
	//ui->list_tutorials_ui();
	ui->populate_tutorials_list();
	//ui->update_tutorial_ui();
	assert(ui->get_size() == 9);
}
